...
datatype pool = pool (price: real) (tick: int) (unlocked: bool) (liquidity: real)
                     (growth: growth) (fee_protocol: fee_protocol) (protocal_fees: \<open>fee \<times> fee\<close>)

hide_const (open) price tick unlocked liquidity growth fee_protocol protocal_fees
...
locale Pool = Tickmap_spec + Tick_spec +
  ...
  and get_pool_protocal_fee_0_\<phi>app[\<phi>synthesis 1000]:
    \<open> \<p>\<r>\<o>\<c> op_get_protocal_fee_0 \<lbrace> s \<Ztypecolon> Pool \<longmapsto> s \<Ztypecolon> Pool\<heavy_comma> fst (pool.protocal_fees s) \<Ztypecolon> \<v>\<a>\<l> \<real> \<rbrace> \<close>
  ...
  and get_pool_protocal_fee_1_\<phi>app[\<phi>synthesis 1000]:
    \<open> \<p>\<r>\<o>\<c> op_get_protocal_fee_1 \<lbrace> s \<Ztypecolon> Pool \<longmapsto> s \<Ztypecolon> Pool\<heavy_comma> snd (pool.protocal_fees s) \<Ztypecolon> \<v>\<a>\<l> \<real> \<rbrace> \<close>
...
definition Uniswap_Pool :: \<open>(fiction, price \<times> tick \<times> bool \<times> liquidity \<times> liquidity \<times> growths \<times> opt_growths \<times> fee_protocol \<times> (fee \<times> fee)) \<phi>\<close>
  where [\<phi>defs]: \<open>Uniswap_Pool x = (
    case x of (price, i, unlocked, Lg, L, growth, \<delta>, fee_proto, protocal_fees) \<Rightarrow>
      ...
      pool price i unlocked (L i) (gSum growth) fee_proto protocal_fees \<Ztypecolon> Pool\<heavy_comma>
...
lemma [\<phi>reason 1200]:
  \<open>(price, i, unlocked, Lg, L, growth, \<delta>, fee_proto, protocal_fees) \<Ztypecolon> Uniswap_Pool \<i>\<m>\<p>\<l>\<i>\<e>\<s>
   pool price i unlocked (L i) (gSum growth) fee_proto protocal_fees \<Ztypecolon> Pool\<heavy_comma>
   (\<lambda>k. 0 < Lg k) \<Ztypecolon> Tickmap\<heavy_comma>
   (Lg, L, growth, i, \<delta>) \<Ztypecolon> Ticks
...
\<Longrightarrow> X \<i>\<m>\<p>\<l>\<i>\<e>\<s> pool price i unlocked (L i) (gSum growth) fee_proto protocal_fees \<Ztypecolon> Pool\<heavy_comma>
        (\<lambda>k. 0 < Lg k) \<Ztypecolon> Tickmap\<heavy_comma>
        (Lg, L, growth, i, \<delta>) \<Ztypecolon> Ticks \<r>\<e>\<m>\<a>\<i>\<n>\<s> R \<a>\<n>\<d> P
\<Longrightarrow> X \<i>\<m>\<p>\<l>\<i>\<e>\<s> (price, i, unlocked, Lg, L, growth, \<delta>, fee_proto, protocal_fees) \<Ztypecolon> Uniswap_Pool \<r>\<e>\<m>\<a>\<i>\<n>\<s> R \<a>\<n>\<d> P\<close>
  unfolding FOCUS_TAG_def
  \<medium_left_bracket> premises _ and I
    I 
    construct\<phi> \<open>(price, i, unlocked, Lg, L, growth, \<delta>, fee_proto, protocal_fees) \<Ztypecolon> Uniswap_Pool\<close>
...
definition \<open>\<Delta>protocal_fees fee_proto zeroForOne L price price' \<gamma>
    = (if zeroForOne
       then if 0 < fst fee_proto
            then (fst (reserve_change zeroForOne L price price') * fee_factor / real (fst fee_proto), 0)
            else (0,0)
       else if 0 < snd fee_proto
            then (0, snd (reserve_change zeroForOne L price price') * fee_factor / real (snd fee_proto))
            else (0,0))\<close>
...
proc swap:
  ...
  output \<open>(price', i', unlocked, Lg, L,
            growth + fee_growth zeroForOne fee_factor L price price',
            \<delta>, fee_protocol,
    ...
    \<s>\<u>\<b>\<j> price' i' transfer_out transfer_in.
      ...
      then amount_specified * (1 - fee) = (if zeroForOne then fst else snd) (reserve_change zeroForOne L price price')
      ...
      \<comment> \<open>The fee growth \<open>growth + fee_growth zeroForOne fee_factor L price price'\<close>
          and the \<open>protocal_fees\<close> are increased accordingly.\<close>
   \<and> transfer_out = (if zeroForOne then snd else fst) (reserve_change zeroForOne L price price')
   \<and> transfer_in = (if zeroForOne then fst else snd) (reserve_change zeroForOne L price price') / (1 - fee)
...
\<close>
  ...
  \<medium_left_bracket> D ;;
    ...
    \<medium_left_bracket>
     ...
     (*FIX ME: ^ there is a bug*)
      ...
      have kk4: \<open>\<not> zeroForOne \<Longrightarrow> next_price < MAX_PRICE\<close> by (smt (verit, best) fee_range price_of_L0 swap_step_next_price_Gt t0 the_\<phi>(16) the_\<phi>(25) the_\<phi>(27) the_\<phi>(33) the_\<phi>lemmata(5))
  ...
  have \<open>zeroForOne \<Longrightarrow>
    growth.map_fee0 (\<lambda>_. I_fee_growth_global (growth + fee_growth zeroForOne fee_factor L price price')) (gSum growth)
      = gSum (growth + fee_growth zeroForOne fee_factor L price price')\<close>
  ...
  have proto_fee': \<open>proto_fee' =
    (if 0 < fee_proto then ((if zeroForOne then fst else snd) (reserve_change zeroForOne L price price')) * fee_factor / real fee_proto else 0)\<close>
    by (smt (verit) fee_factor_def split_def the_\<phi>(16) times_divide_eq_right)

  have proto_fee'_LE0: \<open>0 \<le> proto_fee'\<close>
    unfolding proto_fee' using \<open>0 < price\<close> fee_factor_GT_0 reserve_change'_LE_0' the_\<phi>(1) the_\<phi>(12) the_\<phi>lemmata(6) by force ;;

  if \<open>$zeroForOne\<close> \<medium_left_bracket>
    set_pool_fee_growth_0 ($fee_growth_global)
    is \<open>pool _ _ _ _ (gSum (growth + fee_growth zeroForOne fee_factor L price price')) _ _\<close>
    ...
    if \<open>$protocolFee > 0\<close> \<medium_left_bracket> set_pool_protocal_fee_0 (\<open>fst (pool.protocal_fees _) + $protocolFee\<close>) \<medium_right_bracket>. \<medium_left_bracket> \<medium_right_bracket>. ;;
   ...
   \<medium_right_bracket>. \<medium_left_bracket>
    ...
    if \<open>$protocolFee > 0\<close> \<medium_left_bracket> set_pool_protocal_fee_1 (\<open>snd (pool.protocal_fees _) + $protocolFee\<close>) \<medium_right_bracket>. \<medium_left_bracket> \<medium_right_bracket>. ;;

  \<medium_right_bracket>.
  is \<open>pool price' i' False (L i') (gSum (growth + fee_growth zeroForOne fee_factor L price price')) fee_protocol
           (protocal_fees + \<Delta>protocal_fees fee_protocol zeroForOne L price price' fee_factor)\<close>
  affirm apply (cases protocal_fees; auto simp add: \<Delta>protocal_fees_def fee_proto_def proto_fee' zero_prod_def)
  ...
  $amount0 is \<open>if zeroForOne then fst (reserve_change' L price' price) / (1 - fee)
  ...
  $amount1 is \<open>if zeroForOne then - snd (reserve_change' L price' price)
                    else snd (reserve_change' L price price') / (1 - fee)\<close>
  ...
  if \<open>$zeroForOne\<close> \<medium_left_bracket>
    ...
    $amount0 is \<open>fst (reserve_change' L price' price) / (1 - fee)\<close>
  \<medium_right_bracket>. \<medium_left_bracket>
    ...
    $amount1 is \<open>snd (reserve_change' L price price') / (1 - fee)\<close>
  \<medium_right_bracket>. ;;

  set_pool_unlock (\<open>True\<close>)
  note [[ML_print_depth = 1000]]
  \<medium_right_bracket> apply (auto simp add: \<phi>)
    apply (smt (verit, best) \<open>(if zeroForOne = (0 < amount_specified) then amount_specified - amount_remaining' else amount_calculated') = (if zeroForOne then fst (reserve_change' L price' price) / (1 - fee) else - fst (reserve_change' L price price'))\<close> fee_range nonzero_divide_eq_eq the_\<phi>lemmata(8))
    using \<open>(if zeroForOne = (0 < amount_specified) then amount_calculated' else amount_specified - amount_remaining') = (if zeroForOne then - snd (reserve_change' L price' price) else snd (reserve_change' L price price') / (1 - fee))\<close> the_\<phi>lemmata(8) apply force
    using \<open>(if zeroForOne = (0 < amount_specified) then amount_calculated' else amount_specified - amount_remaining') = (if zeroForOne then - snd (reserve_change' L price' price) else snd (reserve_change' L price price') / (1 - fee))\<close> \<open>0 \<noteq> amount_specified \<and> 0 < price_limit\<close> the_\<phi>lemmata(8) apply force
    using \<open>(if zeroForOne = (0 < amount_specified) then amount_specified - amount_remaining' else amount_calculated') = (if zeroForOne then fst (reserve_change' L price' price) / (1 - fee) else - fst (reserve_change' L price price'))\<close> the_\<phi>lemmata(8) by force .
